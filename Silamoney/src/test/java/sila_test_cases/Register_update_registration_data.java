...
public class Register_update_registration_data extends Base_class {
	...
	public void test_002_registration_update_invalid_email_field() throws Exception {
	UserHandleMessage user = UserHandleMessage.builder().userHandle(handle27).userPrivateKey(reader.getCellData(sheetName, privatekeys, 7)).build();
	...
	public void test_003_registration_update_email_with_invalid_private_key() throws Exception {
	UserHandleMessage user = UserHandleMessage.builder().userHandle(handle27).userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).build();
	EmailMessage message = EmailMessage.builder().uuid(emailUuid).email(updateEmail).build();
	ApiResponse response = api.updateEmail(user, message);

	String Expected_Err_Mgs="Failed to authenticate user signature. The derived address "+Base_class.reader.getCellData(Base_class.sheetName, Base_class.cryptoAddress, 6)+" is not registered to "+Base_class.Handle_27+".";
	...
	public void test_004_registration_update_email_with_not_registered_handle() throws Exception {
...
///////////////////////////////////////////////
	...
	public void test_039_registration_update_address_with_more_than_40_characters_state_name() throws Exception {
	...
	public void test_040_registration_update_address_with_40_characters_state_name() throws Exception {
	...
	Assert.assertEquals(parsedResponse.getStatus(), "SUCCESS");
	Assert.assertEquals(parsedResponse.getMessage(), "Successfully updated entity with handle \""+Handle_27+"\".");
	Assert.assertEquals(parsedResponse.getUserHandle(), Handle_27 );
	...
	@Test(priority = 56)
	@Description("Verify user is not able to update less than 3 characters in entity name.")
	...
	@Test(priority = 57)
	@Description("Verify user is not able to update more than 40 characters in entity name.")
   ...
   @Test(priority = 60)
    ...
    public void test_060_registration_update_birth_date_less_than_18_years() throws Exception {
		int YesterdayDate=0;
		int lastMonth=0;
		if (today ==1) {
		YesterdayDate=28;	
