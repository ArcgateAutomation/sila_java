...
public class Test19_Plaid_sameday_auth extends Base_class{
	...
	public void Test_02_plaid_sameday_auth_with_empty_user_handle() throws Exception {
		ApiResponse response = api.plaidSameDayAuth("", accountName, reader.getCellData(sheetName, privatekeys, 2));
	...
	public void Test_04_plaid_sameday_auth_with_default_account_name() throws Exception {
		...
		Assert.assertEquals(response.getSuccess(), successFalse);
		Assert.assertEquals(((BaseResponse) response.getData()).getStatus(),statusFalse);
		Assert.assertEquals(((BaseResponse)response.getData()).getMessage(),validationMsg3);
	}
	...
	public void Test_06_plaid_sameday_auth_with_Response401() throws Exception {
		ApiResponse response = api1.plaidSameDayAuth(handle22, accountName, reader.getCellData(sheetName, privatekeys, 2));
	...
	public void test_07_plaid_same_day_with_different_private_key() throws Exception {
		ApiResponse response = api.plaidSameDayAuth(handle22, validAccountName, reader.getCellData(sheetName, privatekeys, 7));
