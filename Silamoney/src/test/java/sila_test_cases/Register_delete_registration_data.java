...
public class Register_delete_registration_data extends Base_class {
	...
	//Get Entity
	ApiResponse responseEntiy = api.getEntity(handle26, reader.getCellData(sheetName, privatekeys, 6));
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle("").userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).uuid(emailUuid).build();
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle(handle26).userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).uuid("").build();
	ApiResponse response  = api.deleteRegistrationData(RegistrationDataEnum.EMAIL, message);
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle(handleNotRegistered).userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).uuid(emailUuid).build();
	ApiResponse response = api.deleteRegistrationData(RegistrationDataEnum.EMAIL, message);
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle(handle26).userPrivateKey(reader.getCellData(sheetName, privatekeys, 7)).uuid(emailUuid).build();
	ApiResponse response = api.deleteRegistrationData(RegistrationDataEnum.EMAIL, message);

	Assert.assertEquals(response.getStatusCode(), 403);
	Assert.assertEquals(response.getSuccess(), successFalse);
	Assert.assertEquals(((BaseResponse) response.getData()).getStatus(), statusFalse);
	Assert.assertEquals(((BaseResponse) response.getData()).getMessage().toLowerCase(), "failed to authenticate user signature. the derived address "+reader.getCellData(sheetName, cryptoAddress, 7)+" is not registered to "+Handle_26+".".toLowerCase());
	}
	...
	Assert.assertEquals(((BaseResponse) response.getData()).getMessage(), "No email with UUID "+addressUuid+" exists for user "+Handle_26+".");
	}
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle(handle26).userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).uuid(emailUuid).build();
	ApiResponse response = api.deleteRegistrationData(RegistrationDataEnum.EMAIL, message);
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle(handle26).userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).uuid(phoneUuid).build();
	ApiResponse response  = api.deleteRegistrationData(RegistrationDataEnum.PHONE, message);
	...
	Assert.assertEquals(response.getSuccess(), successTrue);
	Assert.assertEquals(((BaseResponse) response.getData()).getStatus(), statusTrue);
	Assert.assertEquals(((BaseResponse) response.getData()).getMessage(), "No identity with UUID "+emailUuid+" exists for user "+Handle_26+".");
	...
	//Delete data
	DeleteRegistrationMessage message = DeleteRegistrationMessage.builder().userHandle(handle26).userPrivateKey(reader.getCellData(sheetName, privatekeys, 6)).uuid(identityUuid).build();
	ApiResponse response  = api.deleteRegistrationData(RegistrationDataEnum.IDENTITY, message);
